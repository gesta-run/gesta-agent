...
func saveTestOutputClassification(t *testing.T, cfg daemon.Config) {
	t.Helper()
	if err := rulecache.SaveOutputClassificationCache(cfg.DataDir, model.OutputClassificationSettings{
		Revision:      1,
...
func configureCodexHookPolicy(t *testing.T, token string, rule model.PolicyRule) daemon.Config {
	...
	if err := rulecache.SavePolicyCache(cfg.DataDir, []model.PolicyRule{rule}, cfgTime()); err != nil {
