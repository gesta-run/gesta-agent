...
func normalizeGitRemoteURL(raw string) string {
	...
	if hadScheme {
		// URL form: an explicit port sits between host and path and is not
		// part of the repo's identity (gitlab.example.com:2222 via SSH and
		// gitlab.example.com via HTTPS are the same repo).
		...
		// scp-like form (git@host:org/repo.git): the first colon separates
		// host from path.
...
// clampCommitTime pins forged or skewed committer dates: a commit "from the
// future" would otherwise land in a window that does not exist yet.
...
// resolveDefaultBranchRef finds the remote default branch from LOCAL state
// only (never a network call): origin/HEAD when the clone has it, then the
// conventional names, then — for repos wired up via `git remote add` + fetch,
// which never get origin/HEAD — the sole remote-tracking branch if there is
// exactly one. A repo this still cannot resolve is surfaced by the caller as
// scan_errors, never skipped silently.
...
// gitStdout runs git with stdout/stderr separated — combined output would
// interleave warnings into the streams these parsers read — and with a
// scan-sized timeout, since a backfill log is not a 5-second command. stdin,
// when non-empty, feeds --stdin rev lists.
